...
function SW:UpdateLayoutPositions()
	local layout = DoTimerSWDB.settings.layout or "horizontal"
	if layout == "horizontal" then
		for i = 1, 8 do
			local r = self.rows[i]
			if r then
				r.frame:ClearAllPoints()
				r.frame:SetWidth(TARGET_WIDTH)
				r.frame:SetHeight(TARGET_HEIGHT)
				if i == 1 then
					r.frame:SetPoint("TOPRIGHT", self.ui, "TOPRIGHT", 0, 0)
				else
					r.frame:SetPoint("TOPRIGHT", self.rows[i - 1].frame, "TOPLEFT", -6, 0)
				end

				r.nameBtn:ClearAllPoints()
				r.nameBtn:SetWidth(TARGET_WIDTH)
				r.nameBtn:SetHeight(16)
				r.nameBtn:SetPoint("TOP", r.frame, "TOP", 0, -3)
				...
				for j = 1, 8 do
					...
					if j == 1 then
						b:SetPoint("TOP", r.frame, "TOP", 0, -22)
					else
						b:SetPoint("TOP", r.timers[j - 1], "BOTTOM", 0, -5)
					end
				end
			end
		end
		...
		local rowWidth = ICON_SIZE * 8 + 40
		local rowHeight = 62
		for i = 1, 8 do
			local r = self.rows[i]
			if r then
				r.frame:ClearAllPoints()
				r.frame:SetWidth(rowWidth)
				r.frame:SetHeight(rowHeight)
				if i == 1 then
					r.frame:SetPoint("TOPRIGHT", self.ui, "TOPRIGHT", 0, 0)
				else
					r.frame:SetPoint("TOPRIGHT", self.rows[i - 1].frame, "BOTTOMRIGHT", 0, -6)
				end

				r.nameBtn:ClearAllPoints()
				r.nameBtn:SetWidth(TARGET_WIDTH)
				r.nameBtn:SetHeight(16)
				r.nameBtn:SetPoint("TOPRIGHT", r.frame, "TOPRIGHT", 0, -3)
				...
				for j = 1, 8 do
					...
					b:SetHeight(ICON_SIZE)
					if j == 1 then
						b:SetPoint("TOPRIGHT", r.frame, "TOPRIGHT", 0, -22)
					else
						b:SetPoint("TOPRIGHT", r.timers[j - 1], "TOPLEFT", -5, 0)
					end
				end
			end
		end
...
function SW:CreateUI()
	...
	local layoutPanel = CreateFrame("Frame", "DoTimerSWLayoutPanel", UIParent)
	layoutPanel:SetWidth(110)
	layoutPanel:SetFrameStrata("TOOLTIP")
	layoutPanel:SetBackdrop({
		bgFile   = "Interface\\DialogFrame\\UI-DialogBox-Background",
		edgeFile = "Interface\\DialogFrame\\UI-DialogBox-Border",
		tile = true, tileSize = 16, edgeSize = 16,
		insets = { left=4, right=4, top=4, bottom=4 }
	...
	local layoutBtn = CreateFrame("Button", nil, popup)
	layoutBtn:SetWidth(120)
	layoutBtn:SetHeight(22)
	layoutBtn:SetPoint("TOP", popup, "TOP", 0, -58)
	layoutBtn:SetHighlightTexture("Interface\\QuestFrame\\UI-QuestTitleHighlight")
	local layoutBtnFs = layoutBtn:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
	layoutBtnFs:SetAllPoints(layoutBtn)
	layoutBtnFs:SetJustifyH("LEFT")
	layoutBtnFs:SetText((DoTimerSW_L and DoTimerSW_L.BTN_LAYOUT) or "  UI Layout ▶")
	layoutBtn:SetScript("OnClick", function()
		if layoutPanel:IsShown() then
			layoutPanel:Hide()
		else
			scalePanel:Hide()
			layoutPanel:ClearAllPoints()
			layoutPanel:SetPoint("TOPLEFT", popup, "TOPRIGHT", -4, -24)
			layoutPanel:Show()
		end
	end)
	...
	local layoutValues = {
		{ mode = "vertical",   text = (DoTimerSW_L and DoTimerSW_L.LAYOUT_VERT) or "Vertical" },
		{ mode = "horizontal", text = (DoTimerSW_L and DoTimerSW_L.LAYOUT_HORIZ) or "Horizontal" }
	}
	local layoutRows = {}
	for i, v in ipairs(layoutValues) do
		local mode = v.mode
		local txt  = v.text
		local yOff = -8 - (i - 1) * 22
		local row  = CreateFrame("Button", nil, layoutPanel)
		row:SetWidth(94)
		row:SetHeight(20)
		row:SetPoint("TOP", layoutPanel, "TOP", 0, yOff)
		row:SetHighlightTexture("Interface\\QuestFrame\\UI-QuestTitleHighlight")
		local fs = row:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
		fs:SetAllPoints(row)
		fs:SetJustifyH("LEFT")
		...
		row.txt  = txt
		row:SetScript("OnClick", function()
			DoTimerSWDB.settings.layout = mode
			SW:UpdateLayoutPositions()
			SW:Render()
			ClosePopup()
		end)
		layoutRows[i] = row
	end
	layoutPanel:SetHeight((table.getn(layoutValues)) * 22 + 16)

	layoutPanel:SetScript("OnShow", function()
		local cur = DoTimerSWDB.settings and DoTimerSWDB.settings.layout or "horizontal"
		for _, row in ipairs(layoutRows) do
			if row.mode == cur then
				row.fs:SetText("  |cffffd700\226\156\147|r " .. row.txt)
			else
				row.fs:SetText("    " .. row.txt)
			end
		end
	end)
	...
	for i = 1, 8 do
		...
		if target and i <= maxTargets then
			...
			if symbol > 0 then
				...
				row.raidIcon:ClearAllPoints()
				if layout == "horizontal" then					
					row.raidIcon:SetPoint("BOTTOMLEFT", row.name, "TOPLEFT", 0, 2)
				else					
					row.raidIcon:SetPoint("RIGHT", row.name, "LEFT", 0, 0)
				end
				row.raidIcon:Show()
			else
				row.raidIcon:Hide()
			...
			if layout == "horizontal" then
				...
				row.name:SetPoint("RIGHT", row.nameBtn, "RIGHT", 0, 0)
